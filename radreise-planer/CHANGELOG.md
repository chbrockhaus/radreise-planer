...
## v1.12.0 (2026-08-14)
- **Route-Wegpunkte bleiben jetzt auf ihre eigene Etappe begrenzt.** Bisher reichte ein Wegpunkt immer bis zum jeweils nächsten Nachbarn — ohne weitere Wegpunkte war das der Tourstart bzw. das Tourziel, sodass schon der erste gesetzte Wegpunkt auf einer 10-Etappen-Tour die komplette Strecke neu berechnete (gemessen: ~5,5 s, alle 10 Etappen geografisch verändert, obwohl nur eine einzige gemeint war). Etappengrenzen zählen jetzt selbst als Anker in derselben Liste wie Wegpunkte — ein Wegpunkt berührt nur noch den Bereich zwischen seinen unmittelbaren Nachbarn darin. Gemessen: dieselbe Aktion dauert jetzt <1 s, alle nicht betroffenen Etappen bleiben byte-genau unverändert.
- **Ist für eine Etappe ein Campingplatz-Ziel gesetzt, zählt dessen Abzweigpunkt als Anker statt der Etappengrenze** (die Etappe endet ja bereits heute optisch am Camp). Ein Wegpunkt hinter dem Camp bleibt dadurch auf den Bereich zwischen Abzweig- und Wiedereinstiegspunkt der Folge-Etappe begrenzt, statt weitere Etappen zu berühren. Das Camp selbst wird dabei nicht angetastet — seine eigene Routing-Logik (kürzester Weg, Schleifenvermeidung) bleibt unverändert, der Abzweigpunkt dient nur als Grenze.
- Nebeneffekt: Sind bereits Campingplatz-Ziele auf anderen Etappen gesetzt, werden diese durch einen entfernten Wegpunkt jetzt in aller Regel gar nicht mehr berührt (vorher: alle wurden bei jeder größeren Neuberechnung neu berechnet).

## v1.11.6 (2026-08-13)
- **Fix: Ein Camp-Abzweigpunkt konnte nach dem Verschieben eines Route-Wegpunkts an eine völlig falsche Stelle springen** (User-Report: Camp für Etappe 1 gesetzt, dann einen dahinterliegenden Wegpunkt verschoben — der Abzweig landete weit entfernt von der ursprünglichen Position). Ursache: Beim Neuberechnen eines Abschnitts wurden alle Etappengrenzen NACH dem Abschnitt bisher nur pauschal um die Gesamt-Längenänderung des Abschnitts verschoben — das setzte voraus, dass der Abschnitt höchstens EINE Etappengrenze überspannt. Seit Route-Wegpunkte immer den vollen Nachbarabschnitt neu routen (oft die ganze bisher unbearbeitete Tour beim ersten gesetzten Wegpunkt), überspannt ein einziger Vorgang inzwischen regelmäßig mehrere Etappengrenzen gleichzeitig — die pauschale Verschiebung kollabierte dann die Etappenstruktur (Camp-Abzweig sprang in einem Test von km 97 auf km 35, in einem anderen bis auf km 0). Jetzt wird jede betroffene Etappengrenze einzeln per Koordinate auf der neuen Strecke wiedergefunden (dieselbe Technik wie bei „Neu routen"), bleibt also unabhängig von der Zahl der überspannten Etappen an ihrer tatsächlichen geografischen Position.

## v1.11.5 (2026-08-13)
- **Fix: Ein Route-Wegpunkt deutlich abseits der Strecke (z. B. auf der anderen Flussseite) zog die Route direkt danach wieder zur alten Streckenführung zurück** (User-Screenshot: mehrere Wegpunkte auf der linken Rheinseite gesetzt, Route lief trotzdem immer wieder auf die rechte Seite). Ursache: Die Zwischenpunkte für BRouter stammten weiterhin aus der ALTEN Streckenaufzeichnung — nur der gesetzte Wegpunkt selbst wich ab, direkt danach zog die dichte Kette alter Zwischenpunkte (alle 8 km einer) die Route sofort wieder dorthin zurück. Jetzt gilt: Weicht ein Wegpunkt deutlich (> 3 km) von der ursprünglichen Strecke ab, werden für den gesamten Abschnitt beidseitig davon KEINE Zwischenpunkte der alten Route mehr verwendet, sondern nur noch grobe, geradlinige Stützpunkte (alle 40 km) — genug um BRouter über sehr lange Distanzen zu führen, ohne zur alten (falschen) Seite zurückzuziehen. Kleine Korrekturen (≤ 3 km) verhalten sich unverändert wie zuvor.

## v1.11.4 (2026-08-13)
- **Fix: Route-Wegpunkte konnten zu Sackgassen-Schleifen führen** (User-Screenshot: Route fuhr zu einem Wegpunkt hin und auf demselben Weg wieder zurück, statt sinnvoll weiterzuführen). BRouter muss durch jeden Wegpunkt fahren — findet sich dort keine durchgehende Weiterfahrt (z. B. ein Feld- oder Deichweg ohne Anschluss), fährt es dorthin und den gleichen Weg zurück. Betraf Setzen, Ziehen UND das Neuberechnen nach dem Entfernen eines benachbarten Wegpunkts. Solche Stichwege werden jetzt automatisch erkannt und entfernt (dieselbe Technik wie beim Profilwechsel über „Neu routen"), ein Hinweis zeigt die entfernte Strecke. Wird dabei die Sackgasse zum gerade gesetzten/verschobenen Wegpunkt selbst entfernt, schnappt der lila Marker auf den tatsächlich befahrenen Punkt in der Nähe — er bleibt also immer exakt auf der Route.

## v1.11.3 (2026-08-13)
- **Fix: Ein weit gezogener Route-Wegpunkt sprang nach kurzem Abstecher wieder auf die alte Streckenführung zurück**, statt sinnvoll vom neuen Punkt zum Zielpunkt weiterzuführen (User-Screenshot: Wegpunkt von Dinslaken nach Wesel gezogen, Route fuhr trotzdem weiter über Dinslaken). Ursache: Beim Einsortieren des gezogenen Punktes in die Zwischenpunktkette wurde die NEUE Position auf der noch alten Route gesucht — bei einem großen Zug landet dieser Suchtreffer an einer beliebigen, oft weit entfernten Stelle (z. B. nahe dem Streckenanfang), wodurch BRouter zwar kurz zum neuen Punkt sprang, den Rest der Kette aber wieder über die alte Streckenführung bekam. Jetzt wird dafür die ALTE Position (vor dem Ziehen) verwendet — dort liegt der bisherige Abstecher tatsächlich in der aktuellen Route. Betraf nur größere Verschiebungen; kleine Korrekturen in der Nähe des ursprünglichen Punkts waren nicht sichtbar falsch.

## v1.11.2 (2026-08-13)
- **Neu: Route-Wegpunkte routen jetzt immer den vollen Abschnitt zum nächsten Nachbarn neu** (voriger Wegpunkt/Routenstart bis nächster Wegpunkt/Routenziel), nicht nur ein kleines Fenster um den Klick. Setze ich B zwischen Start und Ziel, werden Start→B und B→Ziel komplett neu berechnet; setze ich danach C zwischen B und Ziel, bleibt Start→B unangetastet und nur B→C sowie C→Ziel werden betroffen. Zwischenpunkte aus der bisherigen Strecke werden weiterhin mitgegeben, damit auch ein voller Abschnitt nah am aufgezeichneten Verlauf bleibt — nur direkt um den neuen Wegpunkt weicht die Route ab. **Achtung:** Bei einem ersten Wegpunkt auf einer noch unbearbeiteten, langen Tour kann das die gesamte Strecke leicht verändern (in einem Test: bis zu 3,7 km Abweichung, 17 km kürzer), weil BRouter dann für die ganze Distanz eigene Straßen statt der Aufzeichnung wählt.
- **Fix: Campingplatz-Ziele einer Etappe gingen beim Bearbeiten der Route verloren.** Überlappte der neu berechnete Abschnitt eine Etappe mit gesetztem Camping-Ziel, wurde dieses bisher einfach gelöscht. Jetzt wird es vorher gesichert und nach der Neuberechnung auf der neuen Streckenführung frisch berechnet (dieselbe Technik wie beim Profilwechsel über „Neu routen").
- **Fix: Ein entfernter Wegpunkt konnte die Route nicht mehr exakt in ihre ursprüngliche Bahn zurückführen**, wenn zwischenzeitlich ein anderer, benachbarter Wegpunkt gesetzt worden war — selbst wenn dieser andere Wegpunkt danach auch wieder entfernt wurde. Die gemerkte Originalstrecke wurde beim Überlappen dauerhaft verworfen statt nur vorübergehend blockiert. Jetzt wird beim Entfernen frisch geprüft, ob gerade ein anderer Wegpunkt im Weg steht — verschwindet der Grund für die Blockade, ist die Erinnerung automatisch wieder nutzbar. Getestet: mehrere Wegpunkte setzen und in beliebiger Reihenfolge wieder entfernen ergibt wieder die exakte Original-Route.

## v1.11.1 (2026-08-12)
- **Fix: Der Mauszeiger änderte sich beim Überfahren eines verschiebbaren Punkts nicht.** Die Karte steht wegen Leaflets eingebautem Verschiebe-Modus dauerhaft auf Hand-Cursor (zum Ziehen des Kartenausschnitts). Route-Wegpunkte und die Etappengrenzen-Marker hatten denselben Hand-Cursor gesetzt — beim Überfahren änderte sich also nichts. Jetzt zeigen beide im Ruhezustand den Zeigefinger (wie Leafletts normale Marker), während des Ziehens eine geschlossene Hand als Feedback.

## v1.11.0 (2026-08-12)
- **Fix: Ein Wegpunkt warf viel mehr Strecke weg als nötig.** Beim Setzen eines Wegpunkts hat die App bisher zwei eigene Hilfspunkte 5 km davor und 5 km dahinter auf die Route gesetzt und alles dazwischen von BRouter neu berechnen lassen — auch wenn der Wegpunkt nur 200 m neben der Strecke lag. Gemessen: ein Wegpunkt 217 m neben der Route veränderte 9,1 km Strecke, bis zu 370 m weit weg vom Wegpunkt. Genau das sah aus, als liefe die Route „über Punkte, die man nie gesetzt hat". Das Fenster richtet sich jetzt nach dem Wegpunkt: etwa das Dreifache seines Abstands zur Route, mindestens 600 m, höchstens wie bisher 5 km. Derselbe Fall verändert jetzt nur noch 1,4 km statt 9,1 km; ein weit abseits gesetzter Wegpunkt (3,3 km) bekommt weiterhin den vollen Spielraum.
- **Fix: Nach dem Verschieben eines Wegpunkts stellte sein Entfernen die Originalstrecke nicht mehr her.** Die in v1.10.9 gemerkte Originalstrecke wurde beim Verschieben verworfen, sobald der neue Abschnitt darüber hinausreichte. Sie wird jetzt stattdessen erweitert — außerhalb des bisher gemerkten Bereichs ist die Route ja noch unverändert. Getestet: setzen → verschieben → entfernen und setzen → zweimal verschieben → entfernen ergeben die Route wieder punktgenau wie vorher.
...
## v1.10.7 (2026-08-12)
- **Fix: Beim „Neu routen" gingen selbst gesetzte Route-Wegpunkte verloren.** Die Wegpunktliste für BRouter wurde nur aus einer 8-km-Stichprobe der bisherigen Strecke gebaut — der eben gesetzte lila Wegpunkt war darin nur zufällig enthalten. Nach dem Neuberechnen (typisch: Profilwechsel von „trekking" auf „fastbike") lag er dann neben der Route statt darauf. Jetzt sind selbst gesetzte Wegpunkte feste Zwischenziele und werden immer angefahren; gemessen: Abstand des Wegpunkts zur neu berechneten Route vorher 1,6 km, jetzt 0,3 km (Rest ist BRouters Einrasten aufs Straßennetz).
- **Fix: Zickzack-Spitzen nach einem Profilwechsel.** BRouter muss durch jeden übergebenen Wegpunkt fahren. Nach einem Profilwechsel liegen die Wegpunkte der alten Strecke aber oft auf Wegen, die das neue Profil meidet (Wirtschafts- und Radwege bei „fastbike") — die Route fuhr dann bis zu 2,3 km dorthin und auf demselben Weg wieder zurück. Auf der Karte sah das wie scharfe Zacken aus. Solche Stichwege werden jetzt nach dem Routen erkannt und entfernt: In einem Test (Raesfeld → Poritsch, trekking → fastbike) waren das 40 Stück mit zusammen 79 km reiner Leerkilometer. Ein Hinweis nennt nach dem Neuberechnen die entfernte Strecke. Selbst gesetzte Wegpunkte sind davon ausgenommen — ein Abstecher zu ihnen bleibt erhalten.

## v1.10.6 (2026-08-11)
- **Neu: Größe des Kartenausschnitts direkt in der Karte** — unten rechts über dem Maßstabsbalken, z. B. „3,8 × 5,0 km · Z14" (Breite × Höhe des sichtbaren Bereichs und Zoomstufe). Aktualisiert sich beim Verschieben und Zoomen.
- **Das Protokollfenster zeigt zusätzlich, welcher Bereich tatsächlich abgefragt wird**: „Ausschnitt 3,8 × 5,0 km · Zoom 14 · Maßstabsbalken 500 m" und darunter „Abgefragt: 7,0 × 11 km (auf 0,05°-Raster aufgerundet, 4,1× Fläche)". Der abgefragte Bereich ist bewusst größer als der sichtbare: Er wird auf ein festes Raster aufgerundet, damit kleine Kartenbewegungen dieselbe Abfrage ergeben und aus dem Zwischenspeicher bedient werden können, statt jedes Mal neu zu laden. Im Streckenmodus steht dort stattdessen, wie viele Streckenpunkte mit welchem Umkreis abgefragt werden.
- **Fix: Die Streckenabfrage war doppelt so groß wie beabsichtigt.** Die Punktkette entlang der Route soll auf 60 Punkte ausgedünnt werden; gerechnet wurde die Schrittweite aber abgerundet, sodass sie bei 61–119 sichtbaren Punkten 1 ergab — also gar keine Ausdünnung (gemessen: 110 statt höchstens 60 Punkte). Betroffen war ausgerechnet die aufwendigste Abfrageart. Jetzt wird aufgerundet: 171 sichtbare Punkte ergeben 57 Abfragepunkte.
